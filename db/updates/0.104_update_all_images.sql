-- Changes to images to include md5sum of the image
-- this also uses any queries for views affected by the cascading drop which were introduced since 0.100...sql
-- This field is added because of the peculiar but clever way we use the href field in progress notes to store the pk of the
-- images in the progress note, so that if/when a progress not is saved several times in the consulation, we don't want to
-- continue to re-save the save image
-- If affected by this you must run the modImagesDBI.ImageFix_MD5(..) routine. and then 0.105_update_images.sql

alter table blobs.images add column md5sum text default null;

comment on column blobs.images.md5sum is 'the md5sum of the image imported';


CREATE INDEX "images_md5_idx" ON "blobs"."images"
  USING btree ("md5sum");

drop view blobs.vwPatientImages; -- no cascades


CREATE OR REPLACE VIEW blobs.vwpatientimages AS 
 SELECT images.pk, images.image, images.deleted, images.fk_consult, images.md5sum, consult.consult_date, consult.fk_patient
...
-- deletes vwStaffInclinics then cascades to:
-- clerical.vwtaskscomponents - checked
-- clerical.vwtaskscomponentsandnotes - checked
-- clerical.vwtaskscomponentsnotes - checked.
-- clin_certificates.vwmedicalcertificates - checked.
...
lu_marital.marital, lu_sex.sex, lu_occupations.occupation, lu_ethnicity.ethnicity, lu_languages.language, images.image, images.md5sum, images.fk_consult as fk_consult_image, 
...
-- dops contacts.vwPatients then cascades to:
-- clin_consult.vwpatientconsults  -- checked
-- clin_recalls.vwrecallsdue - checked
-- documents.vwdocuments  -- checked
-- clin_requests.vwrequestsordered
-- documents.vwhl7filesimported 
-- research.diabetes_patients_latest_hba1c
...
CREATE OR REPLACE VIEW contacts.vwpatients AS 
 SELECT 
        ...
        images.pk AS fk_image, images.image, images.md5sum, images.fk_consult as fk_consult_image
...
-- drops contacts.vwpersonsincludingpatients then cacades to
-- clin_history.vwteamcaremembers
-- clin_mentalhealth.vwteamcaremembers
-- clin_referrals.vwreferrals
...
CREATE OR REPLACE VIEW contacts.vwpersonsincludingpatients AS 
 SELECT persons.pk AS fk_person, 
        ...
        towns.town, towns.state, towns.postcode, addresses.fk_town, addresses.preferred_address, addresses.postal_address, addresses.head_office,
         ...
         persons.date_deceased, persons.fk_sex, images.pk AS fk_image, images.image, images.md5sum, images.fk_consult as fk_consult_image
   FROM contacts.data_persons persons
...
drop view clin_procedures.vwImages; -- no cascades from this

CREATE OR REPLACE VIEW clin_procedures.vwimages AS 
 SELECT images.image, images.md5sum, images.deleted as image_deleted, images.fk_consult as fk_consult_image, link_images_procedures.fk_image, link_images_procedures.fk_procedure, link_images_procedures.deleted,
...
insert into db.lu_version (lu_major,lu_minor) values (0, 104)
