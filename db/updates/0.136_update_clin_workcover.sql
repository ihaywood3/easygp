-- new field to keep the latex of the certificate
alter table clin_workcover.visits add column latex text;
comment on column clin_workcover.visits.latex is 'the LaTex definition of the workcover form';
...
CREATE OR REPLACE VIEW clin_workcover.vwworkcover AS 
 ...
 visits.fk_lu_visit_type, visits.diagnosis, lu_systems.system AS coding_system, visits.fk_code, 
        ...
        END AS coded_term, visits.certificate_date, visits.management_plan, visits.review_date,
         visits.assessworkplace, visits.hours_capable, visits.days_capable, visits.restrictions, visits.fk_caused_by_employment,
          visits.doctor_consented, visits.worker_consented, visits.fitness_preinjury_from, visits.fitness_suitable_from, visits.fitness_suitable_to, 
          visits.fitness_unfit_from, visits.fitness_unfit_to, visits.fitness_perm_mod_duties_from, visits.fk_consult, 
          visits.fk_progressnote, visits.fk_coding_system, visits.capabilities, visits.latex, lu_caused_by_employment.caused_by_employment
...
insert into db.lu_version (lu_major,lu_minor) values (0, 136);
