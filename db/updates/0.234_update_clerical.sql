-- change the non-patient payer to a person, previouslyi fk_payer pointed to another patient
alter table clerical.data_patients rename column fk_payer to fk_payer_person;
alter table clerical.data_patients add column fk_payer_branch integer default null;
comment on column clerical.data_patients.fk_payer_person is 
'if the patient does not pay, this is the key to contacts.persons table this could be another patient, or a non-patient in the persons table';
comment on column clerical.data_patients.fk_payer_branch is 
'if the patient does not pay, then the branch of the organisation in data_branches (e.g could be head office) is the payer';
comment on column clerical.data_patients.fk_lu_default_billing_level is
'the default billing level eg private';


Drop view contacts.vwPatients cascade;

-- drop could drop out your billing views as well ... 
...
CREATE OR REPLACE VIEW contacts.vwpatients AS 
 SELECT 
        ...
        patients.fk_doctor, patients.fk_next_of_kin, patients.fk_payer_person, fk_payer_branch, patients.fk_family, patients.medicare_number, 
...
insert into db.lu_version (lu_major,lu_minor) values (0, 234);
