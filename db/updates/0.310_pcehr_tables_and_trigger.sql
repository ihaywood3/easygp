...
alter table clerical.data_patients drop CONSTRAINT data_patients_ihi_check;
alter table clerical.data_patients add constraint ihi_check check (ihi ~ '^800360[0-9]{10}$');
...
CREATE OR REPLACE FUNCTION clin_prescribing.escript_notify() RETURNS trigger AS $xyz$
BEGIN
...
CREATE TRIGGER escript_notify BEFORE INSERT ON clin_prescribing.prescribed
       FOR EACH ROW
       WHEN (NEW.escript_uploaded IS FALSE)
       EXECUTE PROCEDURE clin_prescribing.escript_notify();

create table documents.pcehr_type (
       pk serial primary key,
       "type" text not null
);

comment on table documents.pcehr_type is 'types of PCEHR documents';
insert into documents.pcehr_type (pk,"type") values (1,'Shared Health Summary');
insert into documents.pcehr_type (pk,"type") values (2,'Narrative');
insert into documents.pcehr_type (pk,"type") values (3,'Specialist Letter');
insert into documents.pcehr_type (pk,"type") values (4,'Discharge Summary');
insert into documents.pcehr_type (pk,"type") values (5,'Advanced Care Directive');
grant select on documents.pcehr_type to staff; 


create table documents.pcehr (
       pk serial primary key,
       fk_patient integer references clerical.data_patients (pk) not null,
       "xml" xml not null,
       html text,
       author text not null,
       author_hpii char(16) check (author_hpii ~ '^800361[0-9]+'),
       author_hpio char(16) check (author_hpio ~ '^800362[0-9]+'),
       "local" boolean not null,
       creation timestamp not null default now(),
       accession timestamp,
       fk_lu_pcehr_type integer references documents.pcehr_type(pk) not null
);

grant insert, update on documents.pcehr to staff;

comment on table documents.pcehr is 'table to hold PCEHR documents, both downloaded and generated locally';
comment on column documents.pcehr."xml" is 'XML of the PCEHR document';
comment on column documents.pcehr.html is 'HTML generated by us for display';
comment on column documents.pcehr.author is 'free-text description of document author: clinician name and location';
comment on column documents.pcehr.author_hpii is 'HPI-I of document author';
comment on column documents.pcehr.author_hpio is 'HPI-O of document author';
comment on column documents.pcehr."local" is 'true=created by us and uploaded, false=created elsewhere and downloaded';
comment on column documents.pcehr.creation is 'when created, for remote documents based on internal record of creation';
...
create table documents.access_code_cache (
    ...
    when_requested timestamp default now () not null
);
...
comment on column documents.access_code_cache.backend_pid is 'the PID of the original request for download, so it can identify access code requests';
comment on column documents.access_code_cache.access_code is 'the PCEHR access code, initially NULL as the server won''t know what it is';

truncate table db.lu_version;
insert into db.lu_version (lu_major,lu_minor) values (0, 310);
