-- adjusted the view to allow visualisation of the role that can action a staff task 
-- when a specific person has not been allocated the task.

alter table clerical.task_components alter column fk_staff_allocated drop not null;
...
CREATE OR REPLACE VIEW clerical.vwtaskscomponentsandnotes AS 
 SELECT 
        ...
        END AS pk_view, 
        tasks.related_to AS task, task_components.details, task_component_notes.note, tasks.fk_row, 
        tasks.fk_staff_finalised_task, tasks.fk_staff_must_finalise, tasks.fk_role_can_finalise, 
        ...
        task_components.fk_role, lu_staff_roles.role as role_allocated,
        task_components.pk AS fk_component, task_components.fk_task, 
        task_components.fk_consult, task_components.fk_staff_allocated, task_components.fk_staff_completed, 
        task_components.allocated_staff, task_components.fk_urgency, 
        ...
        lu_urgency.urgency, task_component_notes.pk AS fk_task_component_note, 
   ...
   LEFT JOIN admin.vwstaffinclinics vwstaffinclinics4 On clerical.task_component_notes.fk_staff_made_note = vwstaffinclinics4.fk_staff
   LEFT JOIN admin.lu_staff_roles on clerical.task_components.fk_role = lu_staff_roles.pk
...
COMMENT ON VIEW clerical.vwtaskscomponentsandnotes IS
 'the tasks and eaech component in it''s actioning and any associated notes.
...
insert into db.lu_version (lu_major,lu_minor) values (0, 316);
