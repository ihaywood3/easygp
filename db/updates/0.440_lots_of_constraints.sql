...
ALTER TABLE chronic_disease_management.team_care_arrangements add constraint "team_care_arrangements_fk_person_fkey" foreign key (fk_person) references contacts.data_persons (pk);
ALTER TABLE chronic_disease_management.team_care_arrangements add constraint "team_care_arrangements_fk_employee_fkey" foreign key (fk_employee) references contacts.data_employees (pk);
...
ALTER TABLE defaults.lu_printer_host add constraint "lu_printer_host_fk_clinic_fkey" foreign key (fk_clinic) references admin.clinics (pk);
...
ALTER TABLE clin_consult.lu_shortcut add constraint "lu_shortcut_fk_staff_fkey" foreign key (fk_staff) references admin.staff (pk);
...
ALTER TABLE clin_referrals.referrals add constraint "referrals_fk_pasthistory_fkey" foreign key (fk_pasthistory) references clin_history.past_history (pk);
...
ALTER TABLE drugs.brand add constraint "brand_fk_company_fkey" foreign key (fk_company) references drugs.company (pk);
...
ALTER TABLE chronic_disease_management.team_care_arrangements add constraint "team_care_arrangements_fk_organisation_fkey" foreign key (fk_organisation) references contacts.data_organisations (pk);
...
ALTER TABLE clin_requests.lu_requests add constraint "lu_requests_fk_instruction_fkey" foreign key (fk_instruction) references clin_requests.lu_instructions (pk);
...
ALTER TABLE defaults.script_coordinates add constraint "script_coordinates_fk_lu_printer_host_fkey" foreign key (fk_lu_printer_host) references  defaults.lu_printer_host (pk);
...
UPDATE clerical.data_patients set fk_lu_default_billing_level=1 where fk_lu_default_billing_level=0;  --fixes corrupt data
-- ALTER TABLE ONLY clerical.data_patients ALTER COLUMN fk_lu_default_billing_level SET DEFAULT 1;  -- to be considered
...
UPDATE clerical.data_patients set fk_lu_private_health_fund=1 where fk_lu_private_health_fund=0;  --fixes corrupt data
-- ALTER TABLE ONLY clerical.data_patients ALTER COLUMN fk_lu_private_health_fund SET DEFAULT 1;  -- to be considered
...
-- ALTER TABLE clerical.data_patients add constraint "data_patients_fk_legacy_fkey" foreign key (fk_legacy) references (pk);
-- should NOT be a foreign key, since it is just the patient`s legacy record ID. Might need an additional column and table for fk_legacy_system though
...
ALTER TABLE clin_vaccination.vaccinations add constraint "vaccinations_fk_site_fkey" foreign key (fk_site) references common.lu_route_administration (pk);
...
ALTER TABLE defaults.lu_link_printer_task add constraint "lu_link_printer_task_fk_lu_printer_host_fkey" foreign key (fk_lu_printer_host) references  defaults.lu_printer_host(pk);
ALTER TABLE defaults.lu_link_printer_task add constraint "lu_link_printer_task_fk_task_fkey" foreign key (fk_task) references defaults.lu_printer_task(pk);
...
ALTER TABLE clin_vaccination.lu_vaccines add constraint
"lu_vaccines_fk_description_fkey" foreign key (fk_description)
references  clin_vaccination.lu_descriptions(pk);

ALTER TABLE clin_history.social_history add constraint
"social_history_fk_responsible_person_fkey" foreign key
(fk_responsible_person) references  contacts.data_persons(pk);

ALTER TABLE clin_referrals.inclusions add constraint
"inclusions_fk_referral_fkey" foreign key (fk_referral) references
clin_referrals.referrals(pk);

ALTER TABLE import_export.lu_demographics_field_templates add
constraint "lu_demographics_field_templates_fk_source_program_fkey"
foreign key (fk_source_program) references import_export.lu_source_program(pk);

ALTER TABLE clin_prescribing.prescribed_for_habits add constraint
"prescribed_for_habits_fk_brand_fkey" foreign key (fk_brand)
references  drugs.brand(pk);

ALTER TABLE clin_prescribing.prescribed_for_habits add constraint
"prescribed_for_habits_fk_prescribed_for_fkey" foreign key
(fk_prescribed_for) references  clin_prescribing.prescribed_for(pk);
...
ALTER TABLE clerical.bookings add constraint
"bookings_fk_lu_appointment_status_fkey" foreign key
(fk_lu_appointment_status) references clerical.lu_appointment_status(pk);

ALTER TABLE clerical.bookings add constraint
"bookings_fk_lu_reason_not_billed_fkey" foreign key
(fk_lu_reason_not_billed) references billing.lu_reasons_not_billed(pk);

ALTER TABLE clerical.bookings add constraint
"bookings_fk_lu_appointment_icon_fkey" foreign key
(fk_lu_appointment_icon) references clerical.lu_appointment_icons(pk);

ALTER TABLE defaults.incoming_message_handling add constraint
"incoming_message_handling_fk_blob_fkey" foreign key (fk_blob)
references  blobs.blobs(pk);
