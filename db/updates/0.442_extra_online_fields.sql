alter table billing.items_billed add column inpatient boolean not null default false;
comment on column billing.items_billed.inpatient is 'true if billed as an inpatient service';


alter table contacts.data_numbers add column lspn text;
comment on column contacts.data_numbers.lspn is 'Location-Specific Practice Number. Must be quoted on receipts for radiology and (I think) pathology services. Not used in ordinary General Practice but Medicare may insist we implement.';
...
create or replace view billing.vwitemsandinvoices as
 SELECT vwitemsbilled.pk_items_billed,
    ...
    vwitemsbilled.inpatient as inpatient,
    vwinvoices.lspn as lspn
