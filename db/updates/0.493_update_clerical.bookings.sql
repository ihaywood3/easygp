alter table clerical.bookings add column comment_for_billing text default null;

 COMMENT ON COLUMN clerical.bookings.comment_for_billing is 
 'any additional comments for billing like 
   * item number(s) to be used
   * await histology
   * discuss with doctor etc ';
...
GRANT ALL ON TABLE clerical.vwappointments TO staff;   
 ...
 update db.lu_version set lu_minor = 493;