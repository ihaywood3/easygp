alter table clerical.data_patients add column fk_workplace integer references contacts.data_organisations (pk);
...
CREATE or replace VIEW vwpatients AS
 SELECT
    ...
    patients.fk_workplace,
    org.organisation as workplace
   FROM ((((((((((((((((((((clerical.data_patients patients
     ...
     LEFT JOIN data_addresses addresses ON ((link_person_address.fk_address = addresses.pk)))
     ...
     left join contacts.data_organisations org ON (patients.fk_workplace = org.pk)

);

update db.lu_version set lu_minor=514;
