--Added an arrived column to bookss and the appointments view so Horsts surgery or the ilk can see patients
--in order of the time their bum is on the seat, not the appointment slot used

ALTER TABLE clerical.bookings ADD COLUMN arrived timestamp without time zone default null;
comment on column  clerical.bookings.arrived is 'the ti me the patient arrived for their appointment';
...
CREATE OR REPLACE VIEW clerical.vwappointments AS 
 SELECT bookings.pk,
    ...
    bookings.appointment_specific_memo,
    vwStaff.wholename as staff_name,
    bookings.arrived
...
update db.lu_version set lu_minor=518;