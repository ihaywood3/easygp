-- added a column to allow permant deletion (flag) for unmatched patients 
-- for removing documents not belonging to the surgery

alter table documents.unmatched_patients add column deleted boolean default false;

Comment on column documents.unmatched_patients.deleted is
'If True then this patient (and by implication all associated documents 
 has been marked as deleted... though not physically removed from the table
 they just won''t show up in the inbox
 Was added because of the interminable amount of junk files which should have 
 gone to other practices have accumulated to the point of major annoyance';
...
CREATE OR REPLACE VIEW documents.vwdocuments AS 
 SELECT documents.pk AS pk_document,
    ...
    unmatched_patients.deleted as unmatched_patient_deleted
...
GRANT SELECT ON TABLE documents.vwdocuments TO staff;
 ...
 update db.lu_version set lu_minor=528;
