alter TABLE clin_requests.request_providers add column deleted boolean default false;
DROP VIEW clin_requests.vwrequestproviders;

CREATE OR REPLACE VIEW clin_requests.vwrequestproviders AS 
 SELECT data_organisations.organisation, lu_categories.category,
 lu_request_type.type, data_branches.branch AS default_branch, request_providers.pk AS pk_request_provider, 
 request_providers.fk_branch AS fk_default_branch, request_providers.fk_employee, 
 request_providers.fk_person, request_providers.fk_lu_request_type, 
request_providers.deleted,
 data_branches.fk_organisation, 
 data_branches.deleted AS default_branch_deleted, data_branches.fk_address, data_branches.memo AS default_branch_memo, 
 data_branches.fk_category, data_addresses.street AS default_branch_street,
  data_addresses.deleted AS address_deleted, data_addresses.fk_town, 
  lu_towns.postcode AS default_branch_postcode, lu_towns.town AS default_branch_town, 
  lu_towns.state AS default_branch_state,
   data_addresses1.street, 
  data_addresses1.head_office, lu_towns1.postcode AS headoffice_postcode,
   lu_towns1.town , lu_towns1.state , data_branches1.pk AS fk_branch
...
CREATE OR REPLACE VIEW clin_requests.vwuserproviderdefaults AS 
 ...
 user_provider_defaults.fk_head_office_branch, user_provider_defaults.send_electronically, 
 user_provider_defaults.print_paper, user_provider_defaults.deleted, user_provider_defaults.fk_lu_request_type, 
 data_branches.branch AS default_branch, data_branches.fk_organisation, data_branches.fk_address, 
 data_branches.memo AS default_branch_memo, data_branches.fk_category, data_branches.deleted AS default_branch_deleted, 
 data_organisations.organisation, lu_request_type.type AS request_type, lu_categories.category AS default_category, 
 data_addresses.street AS default_branch_street, lu_towns.town AS default_branch_town, lu_towns.postcode AS default_branch_postcode, 
 data_branches1.branch AS headoffice_branch, data_addresses1.street AS headoffice_street, lu_towns1.postcode AS headoffice_postcode, 
 lu_towns1.town AS headoffice_town
...
insert into db.lu_version (lu_major,lu_minor) values (0, 76)
